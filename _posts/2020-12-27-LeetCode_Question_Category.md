...
- 解法 1：DP
  - 思路：
    - 设`dp[i]`为某结果，`j<i && i%j == 0`，j 的结果已知，那么从 j 的结果推导到 i 有两种路线：
      1. j 的结果重复执行`paste`，则`dp[i] = dp[j] * i/j`
      2. 直接从`i/j`的位置用"补 1"的方式过来，则`dp[i] = dp[i/j] + j`
      3. 综上两种方案，`dp[i]`初始默认值为无穷大，取`min`
    - 由于 1 的特殊性，直接从 2 开始演化，无需为`dp[1]`赋初值
    - 由于`i%j == 0`，所以`j`不会超过`根号i`，所以条件加上`j*j < i`
...
- 解法 2: Math
  ...
  - 注意：
    - 质因数分解时，同一个除数可以使用多次，如`12/2`，所以第二层要用循环

["650. 2 Keys Keyboard" Math]()
...
- 给定整形数组，求其中子串和为 0 的个数，子串允许叠加
  ...
  - 解法 2，prefix sum map
    ...
    - 注意初始化`countMap[0]=1`，如果 0 出现了则自动累加
