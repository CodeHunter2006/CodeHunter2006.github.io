...
- PS: `entry.invalidTime` is a usefull new field, according to best practice.
...
func New(f Func, duration time.Duration) *Memo {
    return &Memo{f: f, duration: duration, cache: make(map[string]*entry)}
}
...
func (memo *Memo) Get(key string) (value interface{}, err error) {
    ...
    if e != nil && e.invalidTime.After(time.Now()) {
        ...
        e = &entry{invalidTime: time.Now().Add(memo.duration), ready: make(chan struct{})}
        memo.cache[key] = e
        memo.mu.Unlock()
