...
| 算法名称              | 稳定性 | 基本流程                                                                                                                                          | 时间复杂度 | 空间复杂度 | 示例                                               | 适用范围                                                     |
| --------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- | -------------------------------------------------- | ------------------------------------------------------------ |
| Quick Sort(快速排序)  | 不稳定 | 综合效率最高。1. 基本思路，递归+分治，每次递归将数据分为左右两边，保证左边比 pivot 小、右边比 pivot 大。2. 通过双指针跳转+循环实现一轮 pivot 调整 | O(nlogn)   | O(logn)    |                                                    | 最常用的排序算法，综合性能最高                               |
| Bucket Sort(桶排序)   | 稳定   | 要求数值在一定范围。1. 根据情况创建 int 数组代表桶，每个桶有一定宽度。2. 遍历源数据，在对应的桶统计                                               | O(n)       | O(n)       | "1833. Maximum Ice Cream Bars" "645. Set Mismatch" | 适合数据范围较小的情况，空间占用最大，始终保持 O(n)          |
| Count Sort(计数排序)  | 稳定   | 要求数值在一定范围。1. 创建对应范围的 int 数组； 2. 遍历源数据，在对应的位置计数                                                                  | O(n)       | O(n)       | "274. H-Index"                                     | 数据范围较小，空间占用大，是一种特殊的(宽度为 1) Bucket Sort |
...
| Merge Sort(归并排序)  | 稳定   |                                                                                                                                                   |            |            |                                                    |                                                              |
| Bubble Sort(冒泡排序) | 不稳定 |                                                                                                                                                   | O(n^2)     | O(1)       |                                                    |                                                              |
