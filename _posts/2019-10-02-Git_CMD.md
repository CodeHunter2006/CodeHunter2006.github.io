...
- 如果某文件之前已经被版本控制了，则增加`.gitignore`并不会影响以前的文件
...
`git checkout tagName/commit_hash`
...
- `-Sstr` 又名 pickaxe，对`str`进行搜索，所有包含特定字符串的增加、修改的 commit 会被列出，用于找到某个特定的字符串涉及的版本。

## reflog

reflog 与远端库没有直接关系，是完全保存在本地的操作记录，通常用来进行回退，即使是`reset --hard`的误操作都可以回退。
...
1. 显示出交互界面——一个 VIM 文本编辑界面(commit 选择/log 编辑)
2. 编辑文本、准备好待执行的命令(修改 commit 前标记/修改 log)
3. 退出编辑`:q`，同时执行编辑中的命令，继续 rebase
4. 中间阶段时，会在 1 ～ 3 之间循环，直到完成 rebase 或放弃这次 rebase
5. 遇到冲突时，需要手动解决冲突，然后`git rebase --continue`
...
`show pop drop`可以加具体缓冲位置为参数`stash@{1}`距离栈顶 1 个距离的位置
...
- `--ignore-rev commit_hash` 忽略某个版本号。比如第一次查看看到一个版本号，想进一步往过去查看，可以忽略已知的版本号。
