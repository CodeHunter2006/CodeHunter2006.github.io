...
将当前工作目录所在 branch 的所有 commit 逐个提出到临时区域，直到找到与 rebase 目标 branch 同根，然后将目标 branch 作为当前 branch 的根，将临时区域的 commit 逐个放入。整个过程 HEAD 将发生逐步移动。
...
以交互模式，将当前的 branch 执行一次 rebase 操作，每次 commit 将被重新给予变更机会。交互模式将以文本修改、执行 rebase 的方式进行。
