...
- 在执行`checkout`动作时，实际上在操作`HEAD`文件，它标记了当前操作的位置，默认是 Staging area，也可以`checkout`到某个历史 commit 进入`detached`只读模式
...
`git remote -v`
查看远端库地址及其名称，origin 就是默认的一个远端库名称。
...
改变 HEAD，从而切换当前工作目录所处的 branch 或 commit。
...
`git checkout commmit_hash`
切换当前状态到一个 commit 快照，进入`detached HEAD`只读模式，通常用来查看某个版本的代码。
