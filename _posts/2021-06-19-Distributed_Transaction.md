...
- 分布式事务满足 BASE 理论，存在中间的**软状态**然后实现**最终一致**，也叫**柔性事务**
...
- 3PC 的缺点：
  ...
  - **数据不一致**如果协调者第三阶段发出 Rollback 请求，而有参与者没有收到，超时后执行了 Commit，则仍然会有数据不一致
...
- 一般步骤：
  ...
  3. 往往最后有小概率的问题需要**人工介入**，通过脚本进行操作，完成最终一致性。但是这种概率已经很低了，并不会影响系统整体效率
