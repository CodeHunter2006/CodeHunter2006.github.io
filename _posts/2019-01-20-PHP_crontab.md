---
layout: post
title:  "（转）ThinkPHP crontab 设定方法"
date:   2019-01-20 20:00:00 +0800
tags: PHP ThinkPHP
---
### 应用场景
...
<br/>
### 基本原理
在每次用户访问时，系统都会检查一遍cron列表，查看是否有需要执行的任务。<br/>
如果需要执行，则立刻执行，然后根据设定的间隔设定下一次执行时间。<br/>
* 优点是不依赖操作系统，任务直接就在项目代码中实现
* 缺点是调用时间不准确，可能被长期拖延

<br/>
### 设定方法
下面的设定方法转自[https://blog.csdn.net/bsf5521/article/details/76522222](https://blog.csdn.net/bsf5521/article/details/76522222)
...
也就是类似使用者的时程表。-u user 是指设定指定 
user 的时程表，这个前提是你必须要有其权限(比如说是 root)才能够指定他人的时程表。
如果不使用 -u user 的话，就是表示设 定自己的时程表。 

参数 : 
crontab -e : 执行文字编辑器来设定时程表，内定的文字编辑器是 VI，
如果你想用别的文字编辑器，则请先设定 VISUAL 环境变数 
