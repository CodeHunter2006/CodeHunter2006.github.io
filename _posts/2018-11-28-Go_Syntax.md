...
- 如果 struct 只有一个成员，那么在初始化时，可以直接传入参数而不用指定成员名称，如`x := TestStruct{param}`
...
- Receiver 为对象类型、指针类型，使用对象、指针都可以调用，编译期会自动使用合适的调用方式
