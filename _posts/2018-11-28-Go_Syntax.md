...
- switch 的目标变量如果忽略，则表示 true，只要后面的 case 表达式为**条件表达式**即可，顺序执行比较
...
chan 的初始化底层实现：`func makechan(t *chantype, size int) *hchan`，它和 map 一样，都是引用型(自动前面加`*`)。
...
- 上面第 3 点，如果把指针或对象赋值给 interface，则调用时会自动进行转换，可以兼容各种类型
